...
/datum/mod_theme/proc/set_skin(obj/item/mod/control/mod, skin)
	...
	for(var/obj/item/part as anything in parts + mod)
		...
		if(length(part.bodyshape_icon_files)) // DOPPLER ADDITION - Icon override bodyshape rendering will not work right without this
			part.bodyshape_icon_files[BODYSHAPE_HUMANOID_T] = used_skin[MOD_WORN_ICON_OVERRIDE] || 'icons/mob/clothing/modsuit/mod_clothing.dmi' // DOPPLER ADDITION
