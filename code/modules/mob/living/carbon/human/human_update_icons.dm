...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if(!found_alt_shape)
			var/fallback_config = dna.species.get_autogen_worn_config(uniform)
			if (!isnull(fallback_config))
				// we found a config for our species? that must mean the clothing doesnt fit. so just trust it
				var/species_icon_file = dna.species.generate_autogen_worn_icon(uniform, src)
...
/mob/living/carbon/human/update_worn_gloves()
	...
	if(!found_alt_shape)
		var/fallback_config = dna.species.get_autogen_worn_config(worn_item)
		if (!isnull(fallback_config))
			// we found a config for our species? that must mean the clothing doesnt fit. so just trust it
			var/species_icon_file = dna.species.generate_autogen_worn_icon(worn_item, src)
			if(species_icon_file)
				icon_file = species_icon_file
