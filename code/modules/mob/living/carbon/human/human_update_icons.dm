...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if(!found_alt_shape)
			var/fallback_config = dna.species.get_custom_worn_config_fallback(OFFSET_UNIFORM, uniform)
			if (!isnull(fallback_config))
				// we found a config for our species? that must mean the clothing doesnt fit. so just trust it
				var/species_icon_file = dna.species.generate_custom_worn_icon_fallback(OFFSET_UNIFORM, uniform, src)
