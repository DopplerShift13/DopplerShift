...
/datum/component/echolocation/Initialize(echo_range, cooldown_time, image_expiry_time, fade_in_time, fade_out_time, images_are_static, blocking_trait, echo_group, echo_icon = "echo", color_path, use_echo = TRUE, show_own_outline = FALSE) // DOPPLER EDIT CHANGE - ORIGINAL: /datum/component/echolocation/Initialize(echo_range, cooldown_time, image_expiry_time, fade_in_time, fade_out_time, images_are_static, blocking_trait, echo_group, echo_icon = "echo", color_path)
	. = ..()
...
/datum/component/echolocation/proc/echolocate()
	if(!COOLDOWN_FINISHED(src, cooldown_last) || stall) // DOPPLER EDIT CHANGE - ORIGINAL: if(!COOLDOWN_FINISHED(src, cooldown_last))
		return
...
/datum/component/echolocation/proc/show_image(image/input_appearance, atom/input, current_time)
	...
	for(var/mob/living/echolocate_receiver as anything in receivers)
		if(!show_own_outline && echolocate_receiver == input) // DOPPLER EDIT CHANGE - ORIGINAL: if(echolocate_receiver == input)
			continue
