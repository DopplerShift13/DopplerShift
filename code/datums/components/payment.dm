...
/datum/component/payment/proc/handle_card(mob/living/user, obj/item/card/id/idcard, total_cost)
	...
	if(!(idcard.registered_account.has_money(total_cost)))
		...
		return FALSE
	target_acc.transfer_money(idcard.registered_account, total_cost, "Port Authority: Usage of Licensed Machinery") // DOPPLER EDIT - NT Removal - ORIGINAL: target_acc.transfer_money(idcard.registered_account, total_cost, "Nanotrasen: Usage of Corporate Machinery")
