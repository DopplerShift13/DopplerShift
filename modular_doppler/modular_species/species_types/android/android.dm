...
/datum/species/android/spec_life(mob/living/carbon/human/target, seconds_per_tick, times_fired)
	...
	if(target.stat == SOFT_CRIT || target.stat == HARD_CRIT)
		target.adjust_fire_loss(1 * seconds_per_tick) //Still deal some damage in case a cold environment would be preventing us from the sweet release to robot heaven
