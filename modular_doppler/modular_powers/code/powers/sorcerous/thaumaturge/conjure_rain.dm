...
/datum/power/thaumaturge/conjure_rain
	...
	Holding a reagent container in hand will consume the chemical and replaces that much of the water with the held reagent. Replacing all the water in a cast will prevent slippery tiles. \
...
/datum/action/cooldown/power/thaumaturge/conjure_rain/use_action(mob/living/user, atom/target)
	...
	if(istype(held_container) && held_container.reagents?.total_volume)
		var/drain_amount = min(buffer.buffer_volume, held_container.reagents.total_volume)
		if(drain_amount > 0)
			...
			if(chem_ratio < 0)
				chem_ratio = 0
			held_container.reagents.trans_to(buffer.reagents, drain_amount, chem_ratio, transferred_by = user)
