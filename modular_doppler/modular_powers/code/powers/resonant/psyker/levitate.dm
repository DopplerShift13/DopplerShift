/datum/power/psyker_power/levitate
	name = "Levitate"
	desc = "Grants the ability to float yourself above surfaces and letting you propel yourself in zero-gravity. Passively drains stress while in use."
...
/datum/action/cooldown/spell/levitate/cast()
	...
	if(!levitating)
		//ADD_TRAIT(owner, TRAIT_IGNORING_GRAVITY, MAGIC_TRAIT)
		...
		//REMOVE_TRAIT(owner, TRAIT_IGNORING_GRAVITY, MAGIC_TRAIT)
...
/datum/action/cooldown/spell/levitate/process(seconds_per_tick)
	...
	if(levitating)
		...
		if(psyker.get_quirk(/datum/quirk/paraplegic)) // There'll probably be several that'd like to do this. Effecively gives a 2x discount
